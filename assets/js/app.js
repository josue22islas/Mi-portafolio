...
(() => {
  ...
  const startThemeTransition = (nextTheme) => {
    ...
    const transitionDuration = window.innerWidth <= 560
      ? 760
      : window.innerWidth <= 1120
        ? 840
        : 920;
    themeButton.style.setProperty("--theme-control-transform", getComputedStyle(themeButton).transform);
    themeButton.classList.add("is-theme-switching");
    let revealAnimation;

    const cleanup = () => {
      // Filled pseudo-element animations outlive their snapshot. Cancel this
      // one so its expanded circle cannot affect the next theme transition.
      revealAnimation?.cancel();
      if (themeViewTransition === transition) {
        themeButton.classList.remove("is-theme-switching");
        themeButton.style.removeProperty("--theme-control-transform");
    ...
    themeViewTransition = transition;

    transition.ready
      .then(() => {
        if (themeViewTransition !== transition) return;
        // Use relative coordinates in the explicitly viewport-sized snapshot.
        // This avoids mixing CSS lengths with the browser's raster scale.
        const iconBounds = themeIcon?.getBoundingClientRect();
        const bounds = iconBounds?.width && iconBounds?.height
          ? iconBounds
        ...
        revealAnimation = root.animate(
          // 150% exceeds the full diagonal even from a viewport corner.
      ...
      .catch(() => {
        // Keep the applied theme if the browser skips a capture (e.g. resize).
